...
export default async function Page({ searchParams }) {
  ...
  return (
    <section>
      ...
      />
      <div className="py-12 px-[6%] md:px-[14%] space-y-16">
        <div
          className="opacity-0 animate-fade-in"
          style={{ animationDelay: "0.2s" }}
        >
          <p className="text-xs md:text-base text-teal mb-8 text-center font-bold uppercase">
            Preencha o formulário ou encontre a unidade mais próxima.
          </p>
          <ContactForm />
        </div>
        <div className="space-y-4">
          <h2 className="text-xs md:text-base text-teal opacity-0 animate-fade-in text-center font-bold uppercase">
            Nossos Endereços e Contatos
          </h2>
          <h3
            className="text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl leading-6 opacity-0 animate-fade-in text-center font-bold uppercase"
            style={{ animationDelay: "400ms" }}
          >
            Encontre o escritório da Socinpro mais perto de você
          </h3>
        </div>
            ...
            <div className="inline-flex whitespace-nowrap space-x-4 lg:flex lg:justify-center lg:space-x-6">
              {locationsTab.map((location) => (
                <Link
                  ...
                  aria-current={activeTab === location.key ? "page" : undefined}
                  className={cn(
          ...
          <div className="flex flex-col items-center justify-center gap-4 bg-gray-100 rounded-sm p-8 lg:px-20 opacity-0 animate-fade-in">
            ...
            <div className="flex flex-wrap justify-center gap-6">
              ...
              {selectedLocation.emails.map((email) => (
                <LinkItem key={email} href={`mailto:${email}`} icon={EmailIcon}>
                  {email}
                </LinkItem>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {selectedLocation.whatsapps.map(
                ({ name, phone, whatsappUrl }) => (
                  <LinkItem key={phone} href={whatsappUrl} icon={WhatsappIcon}>
